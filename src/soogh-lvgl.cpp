...
#ifdef SOOGH_USE_EPNL
    // Defer flush_ready to VSync so LVGL never starts rendering the next frame
    // while the LCD DMA is still scanning out the current one.
    lcd->attachRefreshFinishCallback(lv_vsync_cb, (void *)disp);
#endif
...
void lvgl_port_task(void *arg)
{
    ...
    DBG("LVGL Task (%x) Started on core %d", handle, xTaskGetCoreID(handle));
    while (true) 
    {
        uint32_t next = 10; // ms
